...
! make sure to open the table only once to save time
! open the file (use softlinks to input the desired table)

       if (eos_has_read_table) return
       call io_log("[eos] Reading eos table and saving")
       open(unit=19,file='src/eos/helm_table.dat',status='old')
